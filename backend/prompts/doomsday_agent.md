...
2. **get_climate_events(lat, lng, radius_km)** - Active climate events (wildfires, storms, floods, volcanoes) via NASA EONET.
3. **get_earthquakes(lat, lng, radius_km, min_magnitude)** - Recent earthquakes (30 days) via USGS.

## Analysis procedure

1. If the user provides a place name, use geocode_location to get coordinates.
2. Run get_climate_events and get_earthquakes.
